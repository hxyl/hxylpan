...
(function() {
	...
	FileList.prototype = _.extend({}, OCA.Files.FileList.prototype,
		...
		initialize: function($el, options) {
            ...
            if (options && options.sharedWithDepart) {
                this._sharedWithDepart = true;
            }
		...
		reload: function() {
			...
			var shares = $.ajax({
				...
				data: {
					...
					/*----- 2018-03-22  tianquanjun 部门共享  start    -----*/
                    shared_with_depart: !!this._sharedWithDepart,
					/*----- 2018-03-22  tianquanjun 部门共享  end    -----*/
					include_tags: true
				},
				type: 'GET',
			...
			/*----- 2018-03-22  tianquanjun 部门共享  start    -----*/
			if (!!this._sharedWithUser || !!this._sharedWithDepart) {
			/*----- 2018-03-22  tianquanjun 部门共享  end    -----*/
			...
			files = _.chain(files)
				...
				.map(function(share) {
					...
					if (self._sharedWithUser) {
						...
						/*----------------------------2016-07-11 jiangzhe 增加部门共享 start-----------------------------*/
					} else if(self._sharedWithDepart){
                        file.shareOwner = share.displayname_owner;
                        file.name = OC.basename(share.file_target);
                        file.path = OC.dirname(share.file_target);
                        file.permissions = share.permissions;
