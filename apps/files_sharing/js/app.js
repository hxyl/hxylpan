...
OCA.Sharing.App = {
    ...
    initSharingInDepart: function($el) {
        if (this._inDepartFileList) {
            return this._inDepartFileList;
        }
        this._inDepartFileList = new OCA.Sharing.FileList(
            $el,
            {
                id: 'shares.self',
                scrollContainer: $('#app-content'),
                sharedWithDepart: true,
                fileActions: this._createFileActions()
            }
        );

        this._extendFileList(this._inDepartFileList);
        this._inDepartFileList.appName = t('files_sharing', 'Shared with you');
        this._inDepartFileList.$el.find('#emptycontent').html('<div class="icon-share"></div>' +
            '<h2>' + t('files_sharing', 'Nothing shared with you yet') + '</h2>' +
            '<p>' + t('files_sharing', 'Files and folders others share with you will show up here') + '</p>');
        return this._inFileList;
    },
...
$(document).ready(function() {
    ...
    $('#app-content-sharingdepart').on('show', function(e) {
        OCA.Sharing.App.initSharingInDepart($(e.target));
    });
