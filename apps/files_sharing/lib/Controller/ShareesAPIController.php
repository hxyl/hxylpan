...
class ShareesAPIController extends OCSController {
     ...
     * @param string $search
     */
    protected function getDepartments($search) {
        $this->result['departments'] = $this->result['exact']['departments']  = $departments= [];

        $departments = $this->departmentManager->getDepartmentFullNames($search, null, $this->limit, $this->offset);

        if (!$this->shareeEnumeration || sizeof($departments) < $this->limit) {
            $this->reachedEndFor[] = 'departments';
        }

        $foundUserById = false;
        foreach ($departments as $did => $deptName) {
            if (strtolower($did) === strtolower($search) || strtolower($deptName) === strtolower($search)) {
                if (strtolower($did) === strtolower($search)) {
                    $foundUserById = true;
                }
                $this->result['exact']['departments'][] = [
                    'label' => $deptName,
                    'value' => [
                        'shareType' => Share::SHARE_TYPE_DEPARTMENT,
                        'shareWith' => $did,
                    ],
                ];
            } else {
	...
	protected function searchSharees($search, $itemType, array $shareTypes, $page, $perPage, $lookup) {
        ...
        /**--------------------------------2016-07-11 jiangzhe 增加部门共享 start--------------------------------*/
        // Get department
        if (in_array(Share::SHARE_TYPE_DEPARTMENT, $shareTypes)) {
            $this->getDepartments($search);
        }
        /**--------------------------------2016-07-11 jiangzhe 增加部门共享 end--------------------------------*/
