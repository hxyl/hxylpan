...
class ShareAPIController extends OCSController {
	...
	public function __construct(
		...
		$userId,
		IL10N $l10n,
        IDepartmentManager $departManager
	) {
	...
	protected function formatShare(\OCP\Share\IShare $share, Node $recipientNode = null) {
		...
		if ($share->getShareType() === \OCP\Share::SHARE_TYPE_USER) {
            ...
            /**-----------------------2018-03-21 tianquanjun 部门共享 start--------------------*/
		} else if ($share->getShareType() === \OCP\Share::SHARE_TYPE_DEPARTMENT) {
            $sharewith = $share->getSharedWith();
            $result['share_with'] = $sharewith;
            $departments = $this->departManager->getDepartmentFullNames($sharewith, "Byid");
            $result['share_with_displayname'] = $departments[$sharewith];
            /**----------------------2018-03-21 tianquanjun 部门共享 end-------------------------*/
        }
	...
	public function createShare(
		...
		if ($shareType === \OCP\Share::SHARE_TYPE_USER) {
            ...
            $this->departManager->registerBackend(new \OC\Department\Database());
            if (!$this->shareManager->allowDepartSharing()) {
                throw new OCSNotFoundException($this->l->t('department sharing is disabled by the administrator'));
            }

            // Valid group is required to share
            if ($shareWith === null || !$this->departManager->departExists($shareWith)) {
                throw new OCSNotFoundException($this->l->t('please specify a valid department'));
            }
            $share->setSharedWith($shareWith);
            $share->setPermissions($permissions);
     ...
     * @return DataResponse
     */
    private function getSharedWithDepart($node = null) {
        $departShares = $this->shareManager->getSharedWith($this->currentUser, \OCP\Share::SHARE_TYPE_DEPARTMENT, $node, -1, 0);

        $shares = array_merge($departShares);
	...
	public function getShares(
		...
		if($shared_with_depart === 'true') {
            $result = $this->getSharedWithDepart($path);
            return $result;
        }
        ...
        $departShares = $this->shareManager->getSharesBy($this->currentUser, \OCP\Share::SHARE_TYPE_DEPARTMENT, $path, $reshares, -1, 0);
		if ($this->shareManager->shareProviderExists(\OCP\Share::SHARE_TYPE_EMAIL)) {
		...
		$shares = array_merge($userShares, $groupShares, $linkShares, $departShares,$mailShares, $circleShares);
	...
	protected function canAccessShare(\OCP\Share\IShare $share, $checkGroups = true) {
        ...
        if ($share->getShareType() === \OCP\Share::SHARE_TYPE_DEPARTMENT) {
            // TODO: have a sanity check like above?
            return true;
        }
