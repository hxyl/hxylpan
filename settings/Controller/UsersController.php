...
class UsersController extends Controller {
	 ...
	 */
	public function index($offset = 0, $limit = 10, $gid = '', $pattern = '', $backend = '',$queryType = 0) {

        $users = [];
        if($queryType === 1){

            //通过部门搜索人员
            $batch = $this->userManager->findUsersByDepartId($gid, $limit, $offset);
            if (!empty($batch)) {
     ...
     *
	 */
	public function create($username, $password, array $groups=[], $email='', $did='') {
		...
		try {
			$user = $this->userManager->createUser($username, $password, $did);
