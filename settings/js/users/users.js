...
var UserList = {
	...
	initialize: function($el) {
        ...
        /**--------------------------------2018-03-16 tianquanjun 增加部门共享 start--------------------------------*/
        this.$el.find('.department-admin').singleSelect().on('change', this.onDepartAdminSelect);
        /**--------------------------------2018-03-16 tianquanjun 增加部门共享 end--------------------------------*/
	...
	add: function (user) {
        ...
        /**--------------------------------2018-03-16 tianquanjun 增加部门管理 start--------------------------------*/
        /**
         * department-admin
         */
        var $departAdminSelect = $tr.find('.department-admin');
        if(user.isDepartAdmin){
            $departAdminSelect.find('option').filterAttr('value', 'departAdmminYes').attr('selected', 'selected');
        }else{
            $departAdminSelect.find('option').filterAttr('value', 'departAdmminNo').attr('selected', 'selected');
        }
        /**--------------------------------2018-03-16 tianquanjun 增加部门管理 end--------------------------------*/
        ...
        /**--------------------------------2018-03-16 tianquanjun 增加部门管理 start--------------------------------*/
        $departAdminSelect.on('change', UserList.onDepartAdminSelect);
        /**--------------------------------2018-03-16 tianquanjun 增加部门管理 end--------------------------------*/
    ...
    updateForDept: function (did, limit) {
        if (UserList.updating) {
            return;
        }
        if(!limit) {
        ...
        $.get(
            OC.generateUrl('/settings/users/users'),
            //{ offset: UserList.offset, limit: limit, gid: gid, pattern: pattern, backend:null,departId:did},
            { offset: UserList.offset, limit: limit, gid: did, pattern: pattern, backend:null,queryType:1},
            function (result) {
                var loadedUsers = 0;
                var trs = [];
                //The offset does not mirror the amount of users available,
    ...
    /**--------------------------------2018-03-16 tianquanjun 增加部门管理 start--------------------------------*/
    /**
     * Event handler for when a department has been changed through a single select.
     * This will save the value.
     */
    onDepartAdminSelect: function(ev) {
        var $select = $(ev.target);
        var uid = UserList.getUID($select);
        var departAdmin = $select.val();
        var zTree = $.fn.zTree.getZTreeObj("treeDemo");
        var nodes = zTree.getSelectedNodes();
        var departid;
        if (nodes.length == 0) {
            alert("请先选择一个部门!");
            if(departAdmin == "departAdmminYes"){
                $select.find('option').filterAttr('value', 'departAdmminNo').attr('selected', 'selected');
            }else{
                $select.find('option').filterAttr('value', 'departAdmminYes').attr('selected', 'selected');
            }
            return ;
        }
        ...
        UserList._updateDepartAdmin(uid, departAdmin, departid, function(returnedDepartAdmin){
            if (departAdmin !== returnedDepartAdmin) {
                $select.find(':selected').text(returnedDepartAdmin);
            }
        });
    },

    /**
     * Saves the department for the given user
     * @param {String} [uid] optional user id, sets default quota if empty
     * @param {String} quota department value
     * @param {Function} ready callback after save
     */
    _updateDepartAdmin: function(uid, departAdmin, departid, ready) {
        $.post(
            OC.filePath('settings', 'ajax', 'setdepartadmin.php'),
            {username: uid, departAdmin: departAdmin, departid: departid},
            function (result) {
                if (ready) {
                    ready(result.data.departAdmin);
                }
            }
        );
        /**--------------------------------2016-03-16 tianquanjun 增加部门管理 end--------------------------------*/
    }
};
...
$(document).ready(function () {
    ...
    $('.shareWithDepart').on('click',function (e) {
