...
class OC_Util {
    ...
    /******************** createUuid 2018/03/14 tianquanjun add start **********************/
    /**
     * create uuid.
     *
     * @return uuid
    ...
    /******************** createUuid 2016/03/14 tianquanjun add end **********************/

    /******************** getOnlineNumber 2016/05/26 jiangzhe add start **********************/
    /**
     * load Certs file.
     *
     * @return cerContent
     * @since 7.0.0
     */
    public static function loadCerts()
    {
        //公钥解密
        $pubfxpath =  \OC::$server->getSystemConfig()->getValue("ctrdirectory");
        if (empty($pubfxpath)){
            $pubfxpath =  "/var/www/html/yunpan/config/yunpan.crt";
        }
        $public_key = file_get_contents($pubfxpath); //获取密钥文件内容
        self::$Certs = openssl_x509_parse($public_key);
    ...
    {
        $onlineNum = "1.2.156.15888888.3";

        if (empty($Certs)){
            if (self::loadCerts()){
                $userNum=self::$Certs["extensions"][$onlineNum];
                $tmpUserNum=explode('=',$userNum);
                $userMaxNum=end($tmpUserNum);
     ...
     * @since 7.0.0
     */
    public static function getValidToTime()
    {
        if (empty($Certs)){
            if (self::loadCerts()){
                return self::$Certs["validTo_time_t"];
            };
        }
        return false;
    }
    /******************** getOnlineNumber 2016/05/26 jiangzhe add end **********************/
