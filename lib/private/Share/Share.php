		...
		if (self::isEnabled()) {
			if (!isset(self::$backendTypes[$itemType])) {
				...
				if(count(self::$backendTypes) === 1) {
                    ...
                    \OC_Util::addScript('core', 'sharedialogdepartview');
                    ...
                    \OC_Util::addVendorScript('ztree/js/jquery.ztree.excheck');

                    //\OC_Util::addVendorStyle('ztree/css/jquery-ui-1.10.4.custom.min');
                    \OC_Util::addScript('core', 'popdepartment');
	...
	public static function getItems($itemType, $item = null, $shareType = null, $shareWith = null,
		...
		if (isset($shareType)) {
			...
			if ($shareType == self::$shareTypeUserAndGroups && isset($shareWith)) {
                ...
                $empid = \OC_User::getEmployeeId($shareWith);
                $allUserDeparts = \OC::$server->getDepartmentManager()->getUserFullDeparts($empid); //用户所在部门
                $allDepartId = array();
                foreach (array_keys($allUserDeparts) as $departId) {
                    $allDepartId = \OC::$server->getDepartmentManager()->getSuperDepartmentId($departId); //用户所在部门上级部门
                }
                if (!empty($allDepartId)) {
                    $placeholders = join(',', array_fill(0, count($allDepartId), '?'));
                    $where .= ' OR (`share_type` = ? AND `share_with` IN ('.$placeholders.')) ';
                    $queryArgs[] = self::SHARE_TYPE_DEPARTMENT;
                    $queryArgs = array_merge($queryArgs, array_values($allDepartId));
                }
