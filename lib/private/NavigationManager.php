...
class NavigationManager implements INavigationManager {
	...
	private function init() {
		...
		if ($this->config->getSystemValue('knowledgebaseenabled', false)) {
			$this->add([
				'type' => 'settings',
				'id' => 'help',
				'order' => 5,
		...
		if ($this->userSession->isLoggedIn()) {
			...
			if ($this->isDepartmentadmin()){
                ...
                $this->add([
                    ...
                    'id' => 'departments',
                    'order' => 6,
                    'href' => $this->urlGenerator->linkToRoute('settings_department'),
                    'name' => $l->t('Departments'),
                    'icon' => $this->urlGenerator->imagePath('settings', 'depart.png'),
                ]);
            }
	...
	private function isDepartmentadmin() {
	    $user = $this->userSession->getUser();
	    if($user !== null){
            return $this->groupManager->getSubAdmin()->isDeparmentAdmin($user);
        }
    }
}
