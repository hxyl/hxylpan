...
(function() {
	...
	var TEMPLATE_BASE =
        /**--------------------------------2018-03-20 tianquanjun 增加部门共享 end--------------------------------*/
        '<div class="shareDepartView subView"></div> '  +
        '<div>'  +
        '<input class="shareWithDepart" type="Button" id="shareDepart" '+
        '  value="组织结构"/>' +
        '<input class="shareOrgvalue" type="hidden" id="orgvalue" />' +
        '</div>' +
        /**--------------------------------2018-03-20 tianquanjun 增加部门共享 end--------------------------------*/
	...
	var ShareDialogView = OC.Backbone.View.extend({
        ...
        /** @type {object} **/
        shareDepartView: undefined,

		events: {
			'focus .shareWithField': 'onShareWithFieldFocus',
			'input .shareWithField': 'onShareWithFieldChanged',
			'click .shareWithDepart' : 'onShareDepartClick'
		},
		...
		initialize: function(options) {
			...
			var subViews = {
                ...
                shareDepartView: 'ShareDialogDepartView'
			};
		...
		/*------- 2018-3-20  tianquanjun  部门共享 -------*/
        onShareDepartClick: function(e) {

            e.preventDefault();
            $(e.target).attr('disabled', true);

            this.shareDepartView.$el = this.$el.find('.shareDepartView');

            this.shareDepartView.render(e,this.onCheckedRecipient);

        },

        onCheckedRecipient: function(e, node) {

            this.model.addShare(node, {success: function() {
			...
			$.get(
				...
				function (result) {
					...
					if (result.ocs.meta.statuscode === 100) {
                        ...
                        var departments  = result.ocs.data.exact.departments.concat(result.ocs.data.departments);
						...
						for (i = 0; i < sharesLength; i++) {
							...
							if (share.share_type === OC.Share.SHARE_TYPE_USER) {
                                ...
                                /**-------2016-07-11 jiangzhe 增加部门共享 start ------*/
							} else if (share.share_type === OC.Share.SHARE_TYPE_DEPARTMENT) {
                                departmentsLength = departments.length;
                                for (j = 0; j < departmentsLength; j++) {
                                    if (departments[j].value.shareWith === share.share_with) {
						...
						var suggestions = users.concat(groups).concat(remotes).concat(emails).concat(circles).concat(lookup).concat(departments);
		...
		autocompleteRenderItem: function(ul, item) {
			...
			if (item.value.shareType === OC.Share.SHARE_TYPE_GROUP) {
                ...
                /**-------- 2016-07-11 jiangzhe 增加部门共享 start --------*/
			} else if (item.value.shareType === OC.Share.SHARE_TYPE_DEPARTMENT) {
                text = t('core', '{sharee} (department)', {
                    sharee: text
                });
                /**-------- 2016-07-11 jiangzhe 增加部门共享 end --------*/
            }
            ...
            /**--------------------------------2016-07-11 jiangzhe 增加部门共享 start--------------------------------*/
            if(item.value.shareType === OC.Share.SHARE_TYPE_DEPARTMENT){
                insert.attr('title', text);
            }else{
                insert.attr('title', item.value.shareWith);
            }
            if(item.value.shareType === OC.Share.SHARE_TYPE_GROUP || item.value.shareType === OC.Share.SHARE_TYPE_DEPARTMENT) {
			...
			insert = $("<a>")
				.append(insert);
			return $("<li>")
				.addClass((item.value.shareType === OC.Share.SHARE_TYPE_GROUP | item.value.shareType === OC.Share.SHARE_TYPE_DEPARTMENT) ? 'group' : 'user')
				.append(insert)
				.appendTo(ul);
		},
